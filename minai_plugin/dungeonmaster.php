...
function SetDungeonMasterPrompts($message = "") {
    ...
    if (isset($GLOBALS['enable_prompt_slop_cleanup']) && $GLOBALS['enable_prompt_slop_cleanup']) {        
        $dungeonMasterPrompt = [
            "cue" => ["The Narrator: You should act and respond to the latest dialogue or events. {$GLOBALS["TEMPLATE_DIALOG"]}"]  // leaving this empty ??
        ];
    }
    else {
        $dungeonMasterPrompt = [
            "cue" => [
                "The Narrator: You should act and respond to the latest dialogue or events. {$GLOBALS["TEMPLATE_DIALOG"]}"
            ]
        ];
    }
...
function ProcessDungeonMasterEvent($requestData) {
    ...
    $message = "<OoC_instruction>(Mandatory OoC instruction: " . trim($message) . ")</OoC_instruction>"; // that should be executed unconditionally
