...
function handleGetSingleItem() {
    ...
    if ($item) {
        // Convert PostgreSQL boolean to proper boolean using StandardizeBoolean
        if (isset($item['is_hidden'])) {
            $item['is_hidden'] = StandardizeBoolean($item['is_hidden']);
        }
        if (isset($item['is_available'])) {
            $item['is_available'] = StandardizeBoolean($item['is_available']);
        }
...
function handleGetItemsList() {
    ...
    // Handle is_available and is_hidden independently
    if (isset($_GET['is_available'])) {
        $filters['is_available'] = StandardizeBoolean($_GET['is_available']);
    }
    if (isset($_GET['is_hidden'])) {
        $filters['is_hidden'] = StandardizeBoolean($_GET['is_hidden']);
    }
    ...
    // Ensure consistent boolean representation for is_hidden and is_available
    if (is_array($items)) {
...
function handlePutRequest() {
    ...
    $data = json_decode(file_get_contents('php://input'), true);
    if ($data === null) {
        sendResponse(400, ['success' => false, 'error' => 'Invalid JSON data']);
    }
