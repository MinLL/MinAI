...
function ProcessIntegrations() {
    if (isset($GLOBALS["gameRequest"])) {
        ...
        if (isset($GLOBALS["gameRequest"][3])) {
            ...
            if ((strlen($eventType) > 0) && (strlen($eventData) > 0)) {
    ...
    if (isset($GLOBALS["gameRequest"])) {
        ...
        $s_type = strtolower($GLOBALS["gameRequest"][0] ?? '');
        minai_log("info", "event: $s_type ");
        
        if (isset($GLOBALS["events_to_ignore"]) && in_array($s_type, $GLOBALS["events_to_ignore"])) {
        ...
        if ($s_type == "minai_init") {
            ...
            */
            ...
            $MUST_DIE=true;
        } else if ($s_type == "updatethreadsdb") {
            updateThreadsDB();
            $MUST_DIE=true;
            ...
            $MUST_DIE=true;
        } else if ($s_type == "minai_combatenddefeat") { // Add handling for minai_combatenddefeat
        ...
        if (in_array($s_type, ["npc_talk", "chat"])) { //if ($s_type =="npc_talk") {
            $vars=explode("@",$GLOBALS["gameRequest"][3]);
            $tmp = explode(":", $vars[0]);
            $speaker = $tmp[sizeof($tmp)-1];
            $target = $vars[1];
            $message = $vars[2];
            minai_log("info", "Processing NPC request $s_type: ({$speaker} => {$target}: {$message})");
        ...
        */
        if (in_array($s_type, ["minai_force_rechat","radiant","radiantsearchinghostile","radiantsearchingfriend","radiantcombathostile","radiantcombatfriend"])) { 
            ...
            minai_log("info", "RADIANT event: [{$s_type}] delay={$f_delay} safe={$b_safe} ");
            //error_log("RADIANT event: {$s_type} delay={$f_delay} safe={$b_safe} "); // debug
            if (($s_type == "minai_force_rechat") || $b_safe) {
                // Block rechat/radiant during sex scenes
                if (IsSexActive()) {
                    minai_log("info", "Blocking rechat/radiant/bored during sex scene");
                    //error_log("Blocking rechat/radiant/bored during sex scene");
                    $MUST_DIE = true;
                } elseif (($GLOBALS["HERIKA_NAME"] == "The Narrator") || ($GLOBALS["HERIKA_NAME"] == "Narrator")) {
                    // Fail safe
                    minai_log("info", "WARNING - Radiant dialogue started with narrator. $f_delay ");
                    //error_log("WARNING - Radiant dialogue started with narrator. $f_delay ");
                    $MUST_DIE = true;
                } else {
                    // $GLOBALS["HERIKA_NAME"] is npc1
                    // Fix parsing of target NPC name from radiant event
                    $requestData = ($GLOBALS["gameRequest"][3] ?? '');
                    if (stripos($requestData, 'Context location:') !== false) {
                        // Parse target from format "(Context location: )Min:NPCName"
                        $parts = explode(':', $requestData);
                        $GLOBALS["HERIKA_TARGET"] = trim(end($parts));
                        minai_log("info","request=$requestData target={$GLOBALS["HERIKA_TARGET"]} - exec trace "); // debug
            ...
            if ($s_type != "minai_force_rechat") {
                /* Avoid race condition where we send input, the server starts to process the request, 
                   and then a radiant request comes in 
                */
                if (!$b_safe) {
                    minai_log("info", "Not starting $s_type: Input was too recent: {$f_delay} ".$GLOBALS["input_delay_for_radiance"]);
                    //error_log("Not starting {$s_type}: Input was too recent: {$f_delay} "); //debug
                    $MUST_DIE=true;
                } else 
                    minai_log("info", "OK starting $s_type: no recent input: {$f_delay} - ".$GLOBALS["input_delay_for_radiance"]);
            }
        }
        ...
        if (in_array($s_type,["inputtext","inputtext_s","ginputtext","ginputtext_s","rechat","bored","radiant","minai_force_rechat",
            ...
            //error_log("Setting lastInput $s_type time."); // debug
        ...
        if (($s_type == "minai_storeitem") || ($s_type == "minai_storeitem_batch")) {
        ...
        if ($s_type == "minai_clearinventory") {
        ...
        if (strpos($s_type, "minai_tntr_") === 0) {
        ...
        if (isset($GLOBALS["SEMAPHORES"])) {
            ...
            try {
                ...
                if (isset($semaphore_main) && $semaphore_main) {
                    ...
                    //Logger::warn("[customintegrations] semaphore_main released - exec trace " .__FILE__ . " " . __LINE__);
                }

                if (isset($semaphore_addnpc) && $semaphore_addnpc) {
                    @sem_release($semaphore_addnpc);
                    //Logger::warn("[customintegrations] semaphore_addnpc released - exec trace " .__FILE__ . " " . __LINE__);
                }

                if (isset($semaphore_vsx) && $semaphore_vsx) {
                    @sem_release($semaphore_vsx);
                    //Logger::warn("[customintegrations] semaphore_vsx released - exec trace " .__FILE__ . " " . __LINE__);
                }
            } finally {
